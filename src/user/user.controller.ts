...
export class UserController {
  ...
  @Post()
  createUser(@Body() createUserDto: { username: string; password: string; role: 'admin' | 'student' | 'staff' }): Promise<UserEntity> {
  ...
  @Put(':id')
  @Roles('admin')
  updateUser(@Param('id') id: number, @User() user: any, @Body() updateData: Partial<UserEntity>): Promise<UserEntity> {
    if (user.role === 'admin' || user.userId === id) 
